...
export default function FloatingNavbar() {
  ...
  return (
    <div className="sticky top-0 z-50 w-full px-3 sm:px-6 pt-3 sm:pt-4 pb-2 transition-all duration-300 font-['Plus_Jakarta_Sans']">
      <header 
        className={`max-w-7xl mx-auto transition-all duration-300 ${
          mobileMenuOpen ? 'rounded-3xl' : 'rounded-full'
        } ${
          scrolled 
            ? 'bg-white/95 backdrop-blur-2xl border border-slate-200/90 shadow-xl shadow-slate-200/50 py-2.5 px-4 sm:px-6' 
            : 'bg-white/90 backdrop-blur-xl border border-slate-200/80 shadow-lg py-3 px-4 sm:px-6'
        }`}
      >
        <div className="flex items-center justify-between h-11 sm:h-12">
          ...
          <Link to="/" className="flex items-center gap-2.5">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-2xl bg-gradient-to-br from-pink-500 via-rose-500 to-purple-600 p-[2px] shadow-md shadow-pink-500/20 flex items-center justify-center flex-shrink-0">
              <div className="w-full h-full bg-white rounded-[13px] sm:rounded-[14px] flex items-center justify-center">
                <Heart className="w-4 h-4 text-pink-500 fill-pink-500 animate-pulse" />
              </div>
            </div>
            <div className="flex flex-col text-left">
              <span className="text-base sm:text-lg font-extrabold tracking-tight text-slate-900 font-['Outfit'] flex items-center gap-1">
                Cutiepage
              </span>
          ...
          {/* Mobile Right Controls - Compact Floating Pill Layout */}
          <div className="flex lg:hidden items-center gap-2">
            <button 
              onClick={handleWhatsAppOrder}
              className="bg-gradient-to-r from-emerald-600 to-green-600 text-white text-[11px] font-extrabold py-1.5 px-3 rounded-full flex items-center gap-1 shadow-sm shadow-emerald-500/20"
            >
              <MessageCircle className="w-3.5 h-3.5 fill-white text-white" />
              <span>WhatsApp</span>
            </button>
            ...
            <button 
              ...
              className="p-2 text-slate-700 hover:text-slate-900 rounded-full bg-slate-100/90 border border-slate-200/80 active:scale-95 transition-all"
        ...
        {/* Mobile Floating Drawer Menu inside Glass Capsule */}
        {mobileMenuOpen && (
          <div className="lg:hidden mt-3 pt-3 border-t border-slate-200/80 px-1 pb-3 flex flex-col gap-2 animate-fadeIn text-left">
            {navLinks.map((link) => (
              <Link 
                key={link.name}
                to={link.path}
                className={`text-xs font-bold py-2 px-3.5 rounded-xl transition-all ${
                  isActive(link.path) ? 'bg-pink-50 text-pink-600' : 'text-slate-700 hover:bg-slate-50'
                }`}
              >
                {link.name}
              </Link>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between px-2">
              <Link to="/login" className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                <Lock className="w-3.5 h-3.5 text-slate-500" /> Login
              </Link>
              <button 
                onClick={handleWhatsAppOrder}
